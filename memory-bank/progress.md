...
- 已完成实施计划第 5 步：建立认证与权限数据模型
- 已完成第 5 步认证与权限数据模型增强迁移：`V2__enhance_auth_rbac_model.sql`
- 已完成 `sys_user` 认证扩展字段补充：密码更新时间、最近登录时间、最近登录 IP、锁定时间、禁用原因
- 已完成 `sys_role` 权限扩展字段补充：角色类型、是否内置、排序值、禁用原因
- 已完成 `sys_user_role` 角色绑定扩展字段补充：绑定状态、过期时间、更新审计字段、备注
- 已完成用户状态约束：`ACTIVE`、`DISABLED`、`LOCKED`、`DELETED`
- 已完成角色状态与角色类型约束：角色状态 `ACTIVE`、`DISABLED`，角色类型 `SYSTEM`、`EXTENSION`、`RESERVED`
- 已完成用户角色绑定状态约束：`ACTIVE`、`REVOKED`、`EXPIRED`
- 已完成预留导师角色初始化：`ROLE_MENTOR`，默认状态为 `DISABLED`
- 已完成第 5 步迁移集成测试增强：覆盖 V1/V2 迁移、默认角色种子、多角色绑定与状态约束校验
- 已完成第 5 步 README 中文验证说明更新
- 已完成第 5 步本地验证：PowerShell 环境下通过为 `-Dai.interview.runMigrationTests=true` 参数加引号，及使用 `--%` 停止解析两种方式成功执行迁移集成测试
...
2. 开始实施计划第 6 步，实现注册、登录、退出、获取当前用户信息的基础链路
3. 在第 5 步数据模型基础上接入 JWT、密码加密存储与当前用户查询接口
4. 沿用当前 README 中已验证可用的 PowerShell 命令写法，补充第 6 步中文验证说明

## 6. 里程碑状态

- 里程碑一 基础可运行：进行中（第 1-5 步已完成）
- 里程碑二 求职准备链路可用：未开始
- 里程碑三 面试训练闭环可用：未开始
- 里程碑四 作品集可上线：未开始
